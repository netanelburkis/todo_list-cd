...
module "vpc" {
    ...
    private_subnet_names   = [for k, v in data.aws_availability_zones.available.names : 
                              "todo_list-private-${k+1}"]
    public_subnet_names    = [for k, v in data.aws_availability_zones.available.names : 
                              "todo_list-public-${k+1}"]
    database_subnet_names  = [for k, v in data.aws_availability_zones.available.names : 
                              "todo_list-database-${k+1}"]
